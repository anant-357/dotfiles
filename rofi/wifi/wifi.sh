...
get_match_wifi_list() {
  selection=$(echo -e "$1" | rofi -dmenu -p "$2" -config /home/vix/dotfiles/rofi/wifi/wifi-list.rasi)
  [[ -z "$selection" ]] && exit 1
...
get_match_wifi_on() {
  selection=$(echo -e "$1" | rofi -dmenu -p "$2" -config /home/vix/dotfiles/rofi/wifi/wifi-on.rasi)
  [[ -z "$selection" ]] && exit 1
  does_match_=$(echo -e "$1" | grep "$selection")
  [[ -n "$1" ]] && [[ -z "$does_match_" ]] && exit 1
  echo "$selection"
...
if [[ -z "$enable_test" ]]; then
  ...
  selection=$(get_match_wifi_on "Yes\nNo" "Enable WiFi")
...
selection=$(get_match_wifi_list "$toggle\n$lines\n$create_option" "WiFi")
selected_ssid=$(echo $selection | sed 's/\s\{2,\}/|/g' | awk -F "|" "{print \$$ssid_field}")

#Create new connection
if [ "$selected_ssid" = "$create_option" ]; then
  manual_ssid=$(echo "Enter the SSID of the network." | rofi -dmenu -p "SSID" -config /home/vix/dotfiles/rofi/wifi/config.rasi)
  ...
  if [[ -n "$matches00" ]]; then #If a match exists to the selected networks
    ...
    if [[ "$n_matches00" -eq "1" ]]; then #If only one match, connect
      ...
      chosen_cons00 = $(get_match_wifi_list "$matches00" "Which")
      nmcli con up "$chosen_cons00"
    fi
  else
    #if no matches
    manual_password=$(echo "Enter the Password of the network (or leave blank)." | rofi -dmenu -p "Password" -config /home/vix/dotfiles/rofi/wifi/wifi-pass.rasi)
...
#An existing connection was selected
  ...
  if [[ -n "$matches" ]]; then #If a match exists to the selected networks
    ...
    if [[ "$n_matches" -eq "1" ]]; then #If only one match, connect
      ...
      chosen_cons = $(get_match_wifi_list "$matches" "Which")
    ...
    if [[ -n "$sec0" ]]; then
      user_name=$(echo "Enter identity." | rofi -dmenu -p "Identity" -config /home/vix/dotfiles/rofi/wifi/config.rasi)
      if [[ -z "$user_name" ]]; then
        exit 1
      fi
      password0=$(echo "Enter password of your identity (or leave empty)." | rofi -dmenu -p "Password" -config /home/vix/dotfiles/rofi/wifi/config.rasi)
      n_matches_=$(echo -e "$wlan_" | wc -l)
      #if more than one wlan device let user pick
      if [[ "$n_matches_" -gt "1" ]]; then
        wlan_=$(get_match_wifi_list "$wlan_" "ifname")
      fi
      #add connection
      ...
      if [[ -n "$sec" ]]; then #if network is secured, prompt for password
        password_=$(echo "Enter password of the newtork (or leave blank)." | rofi -dmenu -p "Password" -config /home/vix/dotfiles/rofi/wifi/config.rasi)
      fi
